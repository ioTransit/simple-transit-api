...
func GetRoutesByAgency(c echo.Context) error {
	...
	if found {
		...
		if name == "" {
			return c.JSON(http.StatusOK, map[string]interface{}{
				"name_query":   nil,
				"routes":       gtfs.Gtfs.Routes,
				"routes_count": len(gtfs.Gtfs.Routes),
				"agency_id":    agencyId,
			})
		} else {
...
func GetRouteByAgencyAndRouteId(c echo.Context) error {
	agencyId := c.Param("agencyId")
